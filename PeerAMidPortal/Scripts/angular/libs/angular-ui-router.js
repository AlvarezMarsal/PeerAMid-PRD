...
(function(window, angular, undefined) {
    ...
    function TemplateFactoryProvider() {
         ...
         */
        this.$get = [
            "$http", "$templateCache", "$injector", function($http, $templateCache, $injector) {
    ...
    function $UrlMatcherFactory() {
        ...
        function valToString(val) {
            return val != null
                ? val.toString().replace(/(~|\/)/g, function(m) { return { '~': "~~", '/': "~2F" }[m]; })
                : val;
        }

        function valFromString(val) {
            return val != null
                ? val.toString().replace(/(~~|~2F)/g, function(m) { return { '~~': "~", '~2F': "/" }[m]; })
                : val;
        }
                            ...
                            viewsPromises.push($resolve.resolve(injectables, dst.globals, dst.resolve, state).then(
                                function(result) {
                                    ...
                                    if (isFunction(view.controllerProvider) || isArray(view.controllerProvider)) {
                                        ...
                                        result.$$controller =
                                            $injector.invoke(view.controllerProvider, null, injectLocals);
                                    } else {
