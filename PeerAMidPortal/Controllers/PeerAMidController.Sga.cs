        ...
        try
        {
            foreach (var item in sgaPerformanceModel.SGAPerformanceData)
            {
                if (item.IsTarget == true)
                {
                    ...
                    targetCompany = item.PeerCompanyDisplayName!;
                }
                if ((item.XValue >= 0.75M) && (item.YValue >= 0.75M))
                {
                    companyNames.Add(item.PeerCompanyDisplayName!);
                }
            }

            var bestInClassPhrase = StringExtensionMethods.OxfordComma(companyNames);
            ...
            if (targetXValue < 0.25M)
            {

                if (targetYValue < 0.25M)
                {
                    // SK Condition 1
